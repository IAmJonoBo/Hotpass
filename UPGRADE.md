...
- Quality gate: run `uv run hotpass enrich --input ./tests/data/minimal.xlsx --output /tmp/enriched.xlsx --profile test --allow-network=false`; fail if output missing OR provenance not written OR remote not skipped. (QG-3)
...
Expose all of the above as MCP tools:
	1.	hotpass.refine
	•	input_path, output_path, profile, archive?
	•	runs the CLI refine
	2.	hotpass.enrich
	•	input_path, output_path, profile, allow_network?
	•	MUST run deterministic/local fetchers first, then (only if enabled) Firecrawl-style remote fetch
	3.	hotpass.qa with target=all|contracts|docs
	4.	hotpass.crawl (optional, guarded)
	•	query_or_url, profile, backend=deterministic|firecrawl
...
4.1 “Refine the spreadsheet”
	1.	Find input: look in ./data for .xlsx/.csv; else use provided path.
	2.	Choose profile: load from ./profiles/ (Hotpass-style) — must contain ingest/refine/compliance blocks.
	3.	Call MCP: hotpass.refine(...) → which shells to CLI.
	4.	Store artefact at ./dist/refined.* and return path + quality summary.
...
hotpass.enrich {
  ...
  allow_network: false
	...
	3.	If the row’s confidence < threshold and env has FEATURE_ENABLE_FIRECRAWL_SDK=1 & ALLOW_NETWORK_RESEARCH=1, re-call with allow_network: true to use the translated watercrawl crawler.
...
QG-1: CLI integrity gate
	...
	•	Pass if: exit code 0, lists refine, enrich, qa, contracts, overview
	•	Fail action: regenerate CLI entrypoint, re-run
...
QG-2: Data quality gate
	...
	•	Implementation: either call existing GE context or trigger GitHub Action great_expectations_action.  ￼
	•	Pass if: all expectation suites for the active profile pass.
	•	Fail action: stop pipeline; return GE Data Docs link/HTML to the user.
	•	Rationale: blocks bad or POPIA-violating rows before enrichment. GE is a standard way to “shift left” data governance.  ￼
...
QG-4: MCP discoverability gate
	...
	•	Fail action: restart/repair MCP server; re-register tools
...
QG-5: Docs/instruction gate
	...
	•	Pass if: both files present and non-empty.
...
An upgrade is accepted only when all of these hold:
	...
	2.	Profile completeness: every profile file declares:
	•	ingest (input format, source)
	•	refine (mappings, dedupe, expectations)
	•	enrich (pipeline stages, allow_network default, fetcher chain)
	•	compliance (POPIA/contact rules)
A profile missing any of these fails TA.
	...
	7.	Docs present: .github/copilot-instructions.md contains: “Use uv run hotpass ...”, “Always pass --profile”, “Prefer deterministic enrichment”.

If any TA item fails, the agent must:
	•	fix the failing script / file,
	•	re-run the relevant QG,
	•	then open/update the PR (Copilot /delegate can do this).  ￼

⸻

## 7. Agent Behaviour Notes
	•	Never print or request API keys; read them from env and pass to the process (MCP best practice).  ￼
	•	Ask for tool approval before running shells against user file system (Copilot CLI default).  ￼
	•	Prefer caching MCP tool list during the session for speed; invalidate when CLI changes.  ￼
...
## 8. Provenance (for humans)
	•	GitHub Copilot CLI and agent instructions → how to add MCP, how to read repo-local rules.  ￼

...
- [ ] 2025-11-05 — Continue migrating orchestration pytest assertions to `expect()` helper outside touched scenarios (owner: QA & Engineering).
  - **Progress:** test_error_handling.py completed (46 assertions migrated); 2025-10-31 — compliance verification + enrichment suites migrated to `expect()`. Remaining bare-assert files: 32.

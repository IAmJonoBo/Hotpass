...
def test_profile_lint_json_summary(tmp_path):
    result = _run_linter("--json")
    expect(result.returncode == 0, f"profile_lint --json should succeed (stderr={result.stderr})")
    ...
    expect(summary.get("passed") == summary.get("total"), "All profiles should currently pass lint")
    expect(summary.get("all_passed") is True, "all_passed flag should be True")
    profiles = payload.get("profiles", [])
    expect(isinstance(profiles, list) and profiles, "Profiles list should be populated")
    first = profiles[0]
    expect("name" in first and "passed" in first, "Profile entry should include name and passed fields")


def test_profile_lint_schema_json():

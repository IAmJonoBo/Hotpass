...
def test_run_benchmark_rejects_invalid_runs(tmp_path: Path) -> None:
    try:
        ...
        expect(
            "runs must be a positive integer" in str(exc), "Validation message should mention runs"
        )
...
def test_benchmark_fields_returns_copy() -> None:
    ...
    expect(
        ("Custom", "custom") not in benchmarks.benchmark_fields(), "Returned list should be a copy"
    )

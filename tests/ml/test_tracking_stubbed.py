...
def test_init_mlflow_creates_experiment(fake_mlflow: tuple[ModuleType, _FakeMlflowState]) -> None:
...
def test_init_mlflow_reads_environment(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    expect(state.tracking_uri == "sqlite:///env.db", "Tracking URI should use environment value")
    expect(state.registry_uri is None, "Registry URI remains unset when not provided")
    expect(state.active_experiment == "env_experiment", "Environment experiment should activate")
...
def test_log_training_run_records_payload(
    ...
    expect("file" in record.artifacts and "dir" in record.artifacts, "Artifacts should be logged")
...
def test_log_training_run_formats_metadata_and_skips_missing_artifacts(
    ...
    record = state.runs[run_id]
    expect(record.tags["stage"] == ModelStage.STAGING.value, "Enums should record their value")
    expect(
...
def test_promote_model_archives_existing(fake_mlflow: tuple[ModuleType, _FakeMlflowState]) -> None:
    module, state = fake_mlflow
    ...
    expect(len(prod_versions) == 1 and prod_versions[0].version == "2", "Latest version promoted")
...
def test_promote_model_skips_archiving_when_disabled(
    ...
    expect(len(staging_versions) == 2, "Existing staging versions remain when archiving disabled")


def test_model_loading_and_metadata(fake_mlflow: tuple[ModuleType, _FakeMlflowState]) -> None:
    module, state = fake_mlflow
    ...
    expect(result["loaded"].endswith("/Production"), "Model URI should target production stage")

...
import pandas as pd
from hotpass.column_mapping import (ColumnMapper, infer_column_types,
                                    profile_dataframe)
...
def test_apply_mapping_only_renames_present_columns() -> None:
    ...
    renamed = mapper.apply_mapping(
        df, {"Company": "organization_name", "Email": "contact_email"}
    )
...
def test_profile_dataframe_reports_statistics() -> None:
    ...
    expect(
        profile["duplicate_rows"] == 0, "Duplicate rows should reflect exact matches"
    )
    expect(

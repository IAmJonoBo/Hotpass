...
def test_entity_resolution_feature_applies_linkage(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hotpass.pipeline.features.entity_resolution.add_ml_priority_scores", fake_add_scores
    )
...
def test_entity_resolution_feature_falls_back(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hotpass.pipeline.features.entity_resolution.resolve_entities_fallback", fake_fallback
    )
    monkeypatch.setattr(
        "hotpass.pipeline.features.entity_resolution.add_ml_priority_scores", fake_add_scores
    )
...
def test_enrichment_feature_uses_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hotpass.pipeline.features.enrichment.enrich_dataframe_with_websites", fake_enrich
    )
...
def test_compliance_feature_generates_report(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hotpass.pipeline.features.compliance.add_provenance_columns", fake_add_provenance
    )

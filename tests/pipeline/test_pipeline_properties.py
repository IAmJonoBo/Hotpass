        ...
        st.fixed_dictionaries(
            {
                ...
                "province": st.one_of(
                    st.none(), st.sampled_from(["Gauteng", "Western Cape", "KwaZulu-Natal"])
                ),
                ...
                "website": st.one_of(
                    st.none(), st.sampled_from(["https://example.com", "https://hotpass.example"])
                ),
...
def test_aggregate_group_handles_missing_optional_columns(rows: list[dict[str, Any]]) -> None:
    ...
    expect(isinstance(provenance_raw, str), "selection provenance must serialise to JSON text")
...
_VALID_ROW = st.fixed_dictionaries(
    {
        ...
        "contact_names": st.lists(
            st.text(string.ascii_letters + " -'", min_size=1, max_size=20), min_size=1, max_size=2
        ),
        "contact_roles": st.lists(
            st.sampled_from(["Owner", "Manager", "Pilot"]), min_size=1, max_size=2
        ),
        ...
        "contact_phones": st.lists(
            st.builds(
                lambda digits: "+27" + digits, st.text(string.digits, min_size=9, max_size=9)
            ),
            min_size=1,
            max_size=2,
        ),
...
def test_pipeline_idempotent_with_seed_and_hooks(
    ...
    expect(first_result.refined.equals(second_result.refined), "refined dataframe should be stable")

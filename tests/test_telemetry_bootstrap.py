...
from hotpass.telemetry.bootstrap import (TelemetryBootstrapOptions,
                                         bootstrap_metrics, telemetry_session)
...
def test_bootstrap_options_normalise_attributes() -> None:
    ...
    expect(
        merged_attributes["keep"] == "value", "Original attributes should be preserved."
    )
    expect(
    ...
    expect(
        "invalid" not in settings, "Non-mapping exporter settings should be ignored."
    )
    expect(

...
def test_resolve_options_merges_telemetry(namespace: Namespace) -> None:
    ...
    expect(config.telemetry.service_name == "hotpass-cli", "Service name should override default.")
    expect(config.telemetry.environment == "staging", "Environment should match CLI flag.")
    expect(config.telemetry.exporters == ("otlp",), "Exporter list should prefer CLI values.")
    expect(
